...
### File operations
- Read files with specific paths, not wildcards
- Use targeted edits / search-replace rather than full rewrites
- Discover directory structure before reading files
...
> **Tool name reference** (varies by assistant):
> | Action | Cline / Kilo Code | Cursor | OpenCode |
> |--------|------------------|--------|----------|
> | Read file | `read_file` | `read_file` | `read_file` |
> | Write file | `write` | `edit_file` | `write_file` |
> | Run command | `run_terminal_cmd` | `run_terminal_cmd` | `execute_command` |
> | List dir | `list_dir` | `list_dir` | `list_dir` |
